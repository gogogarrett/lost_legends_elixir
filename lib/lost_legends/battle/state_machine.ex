defmodule LostLegends.Battle.StateMachine do
  @name :StateMachine

  #####
  # External API
  def start_link(battle_id) do
    :gen_fsm.start_link({:local, :"battle:#{battle_id}"}, __MODULE__, [], [])
  end

  def get_state(battle_id) do
    :gen_fsm.sync_send_event(battle_pid(battle_id), :get_state)
  end

  def player_joined(battle_id, player) do
    :gen_fsm.send_event(battle_pid(battle_id), {:player_joined, player})
  end

  def player_left(battle_id, player) do
    :gen_fsm.send_event(battle_pid(battle_id), {:player_left, player})
  end

  def get_players(battle_id) do
    :gen_fsm.sync_send_event(battle_pid(battle_id), :get_players)
  end
  ...
  defp battle_pid(battle_id), do: :"battle:#{battle_id}"
end
